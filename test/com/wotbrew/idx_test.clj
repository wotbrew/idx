...
(deftest indexed-vector-test
  (let [v (vec (range 100))]
    ...
    (let [v2 (auto-idx v)]
      ...
      (is (identical? (-get-index v2 even? :index/hash) (-get-index v2 even? :index/hash)))
      (is (= (filter even? v) (sort (group v2 even?))))
...
(deftest indexed-set-test
  (let [s (set (range 100))]
    ...
    (let [s2 (auto-idx s)]
      ...
      (is (identical? (-get-index s2 even? :index/hash) (-get-index s2 even? :index/hash)))
      (is (= (sort (filter even? s)) (sort (group s2 even?))))
...
(deftest indexed-map-test
  (let [m (zipmap (range 100) (range 100))]
    ...
    (let [m2 (auto-idx m)]
      ...
      (is (identical? (-get-index m2 even? :index/hash) (-get-index m2 even? :index/hash)))
      (is (= (sort (filter even? (vals m))) (sort (group m2 even?))))
